...
import { LayoutDashboard, Bug, LogOut, Search, Plus, Bell, HelpCircle, ChevronDown, User, FolderKanban, ListChecks, BarChart2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden font-sans bg-bg-base text-text-primary">
              ...
              className="w-64 h-9 pl-9 pr-4 rounded-full bg-white/5 border border-white/10 focus:bg-white/10 focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none text-sm transition-all text-white placeholder-text-muted" />
                    ...
                    <div className="relative" ref={notifRef}>
                      <button 
                        ...
                        className="p-2 rounded-full text-text-secondary hover:bg-white/10 transition-colors relative">
                          <Bell size={18} />
                      ...
                      {isNotificationsOpen && (
                        <div className="absolute right-0 mt-3 w-80 glass-panel rounded-xl py-2 z-50 border border-white/10 shadow-2xl origin-top-right animate-in fade-in slide-in-from-top-2 max-h-96 flex flex-col">
                          <div className="px-4 py-3 border-b border-white/10 shrink-0 flex justify-between items-center">
                            <p className="text-sm font-semibold text-white">Notifications</p>
                            <span className="text-xs text-brand-primary cursor-pointer hover:underline" onClick={fetchNotifications}>Refresh</span>
                          </div>
                            ...
                            {notifications.length > 0 ? notifications.map(notif => (
                              <div key={notif.id} 
                                ...
                                <div className="flex justify-between items-start mb-1">
                                  <p className={cn("text-sm", !notif.read ? "text-white font-medium" : "text-text-secondary")}>
                                    {notif.message}
                                  </p>
                                  {!notif.read && <div className="w-2 h-2 rounded-full bg-brand-primary mt-1 shrink-0"></div>}
                                </div>
                                <p className="text-[10px] text-text-muted">{new Date(notif.createdAt).toLocaleString()}</p>
                              </div>
                            )) : (
            ...
            <div className="absolute right-0 mt-3 w-56 glass-panel rounded-xl py-2 z-50 border border-white/10 shadow-2xl origin-top-right animate-in fade-in slide-in-from-top-2">
                                ...
                                <Link to="/profile" className="w-full text-left px-4 py-2 text-sm text-text-secondary hover:text-white hover:bg-white/10 transition-colors flex items-center gap-2">
            ...
            <div className="flex flex-1 overflow-hidden relative">
              ...
              to="/backlog"
              className={cn(
                "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-sm font-medium group relative overflow-hidden mt-1",
                location.pathname === '/backlog' ? "text-white" : "text-text-secondary hover:text-white hover:bg-white/5"
              )}>
              
                            {location.pathname === '/backlog' &&
              <div className="absolute inset-0 bg-gradient-to-r from-orange-500/20 to-transparent -z-10"></div>
              }
                            {location.pathname === '/backlog' &&
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-orange-500 rounded-r-md shadow-[0_0_10px_rgba(249,115,22,0.8)]"></div>
              }
                            <ListChecks size={18} className={location.pathname === '/backlog' ? "text-orange-500" : "text-text-muted group-hover:text-text-secondary transition-colors"} />
                            <span>Backlog</span>
                        </Link>
              ...
              className={cn(
                "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-sm font-medium group relative overflow-hidden mt-1",
                location.pathname === '/reports' ? "text-white" : "text-text-secondary hover:text-white hover:bg-white/5"
              )}>
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-green-500/20 to-transparent -z-10"></div>
                            ...
                            {location.pathname === '/reports' &&
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-green-500 rounded-r-md shadow-[0_0_10px_rgba(34,197,94,0.8)]"></div>
              }
                            <BarChart2 size={18} className={location.pathname === '/reports' ? "text-green-500" : "text-text-muted group-hover:text-text-secondary transition-colors"} />
                            <span>Reports</span>
                        </Link>
              ...
              className={cn(
                "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-sm font-medium group relative overflow-hidden mt-1",
                location.pathname === '/projects' ? "text-white" : "text-text-secondary hover:text-white hover:bg-white/5"
              )}>
              
                            {location.pathname === '/projects' &&
              <div className="absolute inset-0 bg-gradient-to-r from-brand-accent/20 to-transparent -z-10"></div>
              }
                            {location.pathname === '/projects' &&
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-brand-accent rounded-r-md shadow-[0_0_10px_rgba(6,182,212,0.8)]"></div>
              }
                            <FolderKanban size={18} className={location.pathname === '/projects' ? "text-brand-accent" : "text-text-muted group-hover:text-text-secondary transition-colors"} />
                            <span>Projects</span>
                        </Link>
