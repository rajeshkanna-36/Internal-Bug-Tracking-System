...
export const BugDrawer = ({ isOpen, onClose, bugId }) => {
  const { user } = useAuth();
  ...
  const handleCreateSubTask = async () => {
    if (!newSubTaskTitle.trim() || !bug) return;
    setSubmittingSubTask(true);
    try {
      const requestPayload = {
        title: newSubTaskTitle,
        description: "Sub-task for " + bug.title,
        priority: bug.priority,
        issueType: bug.issueType === 'EPIC' ? 'STORY' : 'TASK',
        projectId: bug.project?.id,
        parentBugId: bug.id
      };
      const response = await api.post('/api/bugs', requestPayload);
      setBug({
        ...bug,
        subTasks: [...(bug.subTasks || []), response.data]
      });
      setNewSubTaskTitle("");
    } catch (error) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const response = await api.uploadAttachment(bug.id, file);
      setBug({
        ...bug,
        attachments: [...(bug.attachments || []), response.data]
      });
    } catch (error) {
        ...
        onClick={onClose} />
          ...
          <>
                ...
                <div>
                                    ...
                                    {(bug.issueType === 'EPIC' || bug.issueType === 'STORY' || bug.subTasks?.length > 0) && (
                                      <div className="pt-8 border-t border-white/10">
                                          <h3 className="text-sm font-semibold text-text-secondary mb-5 uppercase tracking-wider">Child Issues</h3>
                                          
                                          {bug.subTasks?.length > 0 && (
                                            <div className="space-y-2 mb-4">
                                              {bug.subTasks.map(child => (
                                                <div key={child.id} className="flex items-center justify-between bg-white/5 border border-white/10 p-3 rounded-lg hover:bg-white/10 transition-colors group cursor-pointer">
                                                  <div className="flex items-center gap-3">
                                                    <IssueTypeIcon type={child.issueType} />
                                                    <span className="text-sm text-white font-medium group-hover:underline">BUG-{child.id}</span>
                                                    <span className="text-sm text-text-secondary">{child.title}</span>
                                                  </div>
                                                  <div className="flex items-center gap-4">
                                                    <PriorityIcon priority={child.priority} />
                                                    <span className={cn("text-[10px] px-2 py-0.5 rounded uppercase font-bold", statusConfig[child.status]?.color, statusConfig[child.status]?.bg)}>
                                          ...
                                          <div className="flex gap-2">
                                            <input 
                                              ...
                                              className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-brand-primary/50"
                                            />
                                            <button 
                                              ...
                                              {submittingSubTask ? 'Adding...' : 'Add child issue'}
                                            </button>
                                          </div>
                                      </div>
                                    )}
                                    ...
                                    <div className="pt-8 border-t border-white/10">
                                      <div className="flex items-center justify-between mb-5">
                                        <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider">Attachments</h3>
                                        <label className="bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors cursor-pointer disabled:opacity-50">
                                      ...
                                      {bug.attachments?.length > 0 ? (
                                        <div className="grid grid-cols-2 gap-3">
                                          {bug.attachments.map(att => (
                                            <a 
                                              key={att.id} 
                                              href={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}/api/bugs/${bug.id}/attachments/${att.id}`}
                                              target="_blank"
                                              rel="noreferrer"
                                              className="flex items-center gap-3 bg-white/5 border border-white/10 p-3 rounded-lg hover:bg-white/10 transition-colors group cursor-pointer">
                                              <div className="p-2 rounded bg-brand-primary/20 text-brand-primary">
                                                <BookOpen size={16} />
                                              </div>
                                              <div className="flex-1 overflow-hidden">
                                                <p className="text-sm text-white font-medium truncate group-hover:underline">{att.fileName}</p>
                                                <p className="text-[11px] text-text-muted">{att.fileType}</p>
                                              </div>
                                            </a>
                                          ))}
                                        </div>
                                        ...
                                        <div className="text-sm text-text-muted text-center py-8 border border-dashed border-white/10 rounded-xl bg-white/5">
                                            No attachments yet. Click the upload button to add files.
                                        </div>
                                      )}
                                    </div>
                    ...
                    comments.map((comment) => {
                      const isSystemComment = comment.system || comment.isSystem;
                      if (isSystemComment) {
                        return (
                          <div key={comment.id} className="flex items-center gap-4 py-2 opacity-80">
                            <div className="w-9 h-9 flex items-center justify-center shrink-0">
                                <div className="w-2 h-2 rounded-full bg-text-muted"></div>
                            </div>
                            <div className="flex-1 text-sm text-text-muted">
                                <span className="font-semibold text-text-secondary">{comment.user?.name || comment.user?.username}</span>
                                {' '}{comment.text}{' '}
                                <span className="text-[11px] text-white/30 ml-2">{new Date(comment.createdAt).toLocaleString()}</span>
                            </div>
                          </div>
                        );
                      }
